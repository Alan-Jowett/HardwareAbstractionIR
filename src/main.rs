...
fn render_host_serial_register_model_slice(items: &[HostSerialRegisterModel]) -> String {
    let rendered = items
        ...
        .map(|item| {
            format!(
                "SerialRegisterModel {{ driver_id: {}, sr_address: 0x{:X}u64, dr_address: 0x{:X}u64, txe_mask: 0x{:08X}u32, tc_mask: 0x{:08X}u32, rxne_mask: 0x{:08X}u32 }}",
...
fn host_emulator_wires_hal_and_companion_state() {
    ...
    let spi = host.spi1_emulator();
    spi.push_received_bytes(&[0x11, 0x22]).expect("inject spi");
    assert!(spi.take_transmitted_bytes().is_empty());

    let i2c = host.i2c1_emulator();
    i2c.push_received_bytes(&[0x33]).expect("inject i2c");
    assert!(i2c.take_transmitted_bytes().is_empty());
}
"#,
        )
