...
- **Examined:** manifest, repository schema/profile docs, exact-variant manifest, official datasheet pages 7 and 29-31, official header/startup/linker assets, official ADC/TIM DMA examples, exact-family SVD, exact-variant YAML, and transitive family/peripheral/interrupt/DMA YAML fragments reachable from that approved source
- **Method:** manifest-grounded clean extraction; exact-variant pruning of family-level structural/profile data; package-aware pin-route filtering; adversarial comparison against the neighboring C8T6 bundle as prior art only; schema validation; Embassy generator translation-boundary check
...
- **Limitations:** official vendor documents are family-level; some family-shared register fields remain structurally present even when the exact variant omits the corresponding peripheral; GPIO and USB Embassy omissions are explicit rather than guessed around
