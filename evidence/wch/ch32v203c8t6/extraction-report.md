...
| `ch32-rs-ch32v203xx-svd` + `ch32-rs-ch32v203c8t6-yaml` | Field descriptions | Yes | Partially | Yes | 396 remaining gaps, led by AFIO, EXTI, RCC, TIM2/3/4, ADC1/2, DMA1, and a 7-field `USBFSH` host-side tail. | Normalization mismatch / representational difference. |
| `ch32-rs-ch32v203xx-svd` + `ch32-rs-ch32v203c8t6-yaml` | Field access modes | Yes | Partially | Yes | 2129 fields still lack explicit access semantics, including most AFIO/TIM/GPIO/ADC families and many imported USB fields outside the explicit read-only status bits. | Partial source sparsity plus normalization mismatch / representational difference. |
...
| `ch32-rs-ch32v203c8t6-yaml` transitive USB fragments | Field enumerated values | Yes | Partially | Yes | 25 USB fields now carry enums; 2419 fields still lack approved-source enum tables. | Partial source sparsity. |
