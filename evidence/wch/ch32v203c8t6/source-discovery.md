...
3. Checked the local repository for pre-existing CH32V203 evidence artifacts and found none.
...
6. Rejected sources that were missing, indirect, board-specific, or lower-authority than a vendor-published alternative.
...
| `openwch-ch32v20x-sdk` | `sdk` | https://github.com/openwch/ch32v20x | official | family-level | high | Official WCH repository with SDK/examples and package-specific project assets. Useful for cross-checking names and implementation-facing details. |
| `openwch-ch32v20x-header` | `vendor-header` | https://raw.githubusercontent.com/openwch/ch32v20x/main/EVT/EXAM/SRC/Peripheral/inc/ch32v20x.h | official | family-level with exact D6 subgroup mapping | high | Header explicitly maps `CH32V20x_D6` to `CH32V203C8`, which is the strongest variant-near machine-readable source found. |
| `openwch-ch32v20x-startup-d6` | `source-code` | https://raw.githubusercontent.com/openwch/ch32v20x/main/EVT/EXAM/SRC/Startup/startup_ch32v20x_D6.S | official | family-level with exact D6 subgroup mapping | medium | Useful cross-check for vector table ordering and handler names; narrower value than the header or manuals. |
...
| Official SVD / ATDF / IP-XACT / SystemRDL | No official machine-readable register description was found from WCH or `openwch`. | Use the WCH manuals plus official header/startup code. |
...
- No official SVD / ATDF / IP-XACT / SystemRDL source identified yet.
- Family-level manuals may still leave some package-specific nuances implicit rather than explicit.
- Core-level semantics may depend on the processor manual if the device-family manuals are thin.

## Coverage

- **Examined:** HAIR repo grounding docs, WCH download references, official `openwch/ch32v20x` repository, official vendor header/startup files.
- **Method:** schema grounding, local repo search, targeted vendor-first web discovery, authority filtering, and adversarial source falsification.
- **Excluded:** community SVDs/mirrors, board-only collateral, and any source that could not be verified as official or actually present.
- **Limitations:** WCH download pages are JavaScript-heavy, so the page wrapper is easier to verify than the PDF metadata itself; the official docs appear to be family-level rather than exact-part-only.

## Follow-Up Recommendations

1. If you later obtain local PDFs, add them as local `path` sources so extraction does not depend on live vendor URLs.
2. During extraction, cross-check any package-specific pin or memory claims against both the datasheet and the D6 header/startup grouping before treating them as exact.
3. Re-run discovery if WCH publishes an official errata document or machine-readable register description for CH32V203.
