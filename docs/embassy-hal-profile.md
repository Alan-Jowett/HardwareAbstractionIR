...
| `i2c` | supported | Requires explicit pin-routing data. Portable master traits additionally require an explicit family selector plus `i2cMasterBindings`; HAL-specific slave APIs additionally require an explicit family selector plus `i2cSlaveBindings`. Async master/slave surfaces also require the matching interrupt closure. The first supported I2C families are limited to 7-bit controller-master transactions and 7-bit single-own-address slave packet transactions. |
...
| `interrupt` | supported | Generates IRQ enums/bindings from the device interrupt inventory plus `interruptTopology`. An `interrupt` driver may additionally expose MCU-wide reset only when it carries `systemResetOperationRef`. |
...
| `i2c` | `pinTopology.routes`; clock/reset support for emitted bring-up helpers; and, for approved I2C lowering families, an explicit family selector plus either `i2cMasterBindings` naming the direct START/STOP/ACK/data/status/address-clear roles used by a master transaction path or `i2cSlaveBindings` naming the own-address, direction, packet-boundary, data, and clear roles used by a slave packet path. Interrupt routes are required when the emitted API claims async master or async slave behavior, and DMA routes only when a later approved I2C family names a DMA-backed transaction path explicitly |
...
| `interrupt` | `structure.device.interrupts`, `profiles.mcuSoc.interruptTopology.routes`, and the referenced `interruptTopology.sources`; plus any clear/ack operations or control refs required by emitted helper methods. An MCU-wide reset additionally requires `systemResetOperationRef` to name one explicit semantic operation on the interrupt-controller path whose terminal write has an explicit target and value. |
...
| `i2c` | Bring-up helpers and only those bus transaction methods whose start/address/data/stop behavior is explicitly modeled. When `loweringPattern = "legacy-event-i2c-master"`, the generated surface may implement blocking `embedded_hal::i2c::I2c<SevenBitAddress>` for the approved 7-bit controller-master path, and may additionally implement `embedded_hal_async::i2c::I2c<SevenBitAddress>` only when the same driver instance claims `embedded-hal-async-i2c-master` and names the required interrupt-enable and event/error route closure. When `loweringPattern = "legacy-event-i2c-slave"`, the generated surface may instead implement HAL-specific blocking and async 7-bit slave packet APIs whose naming and shape stay as close as practical to the generated master surface while still making own-address setup, packet direction, packet completion, and optional ISR-level dispatch explicit. Async slave behavior is gated by capability tag `embassy-async-i2c-slave`, and ISR-level dispatch is a further opt-in on top of that async slave contract. Slave mode remains limited to one configured own address, packet boundaries at STOP or repeated START, and RX-from-master / TX-to-master behavior; general-call, dual-address, SMBus, and 10-bit addressing remain out of subset. |
...
| `interrupt` | IRQ enums plus bind/clear/ack helpers justified by the interrupt inventory, routes, and source-level operations; and, when tagged `embassy-time-driver` with `timeDriverSource = "systick"`, the generated SysTick-backed async time-base support module. When `systemResetOperationRef` is present and valid, the module additionally exposes `system_reset() -> !`, which performs the explicitly modeled MCU-wide reset write and does not return. This API restarts the MCU through its documented reset path; it does not promise to power-cycle or reset external board components. |
...
13. If an `i2c` driver instance names `loweringPattern = "legacy-event-i2c-master"`,
    the same driver instance must also carry explicit `i2cMasterBindings`
    naming the direct START/STOP/ACK/data/status/address-clear roles used by
    the portable 7-bit controller-master transaction path. The selected family
    may not be reconstructed from vendor register names or from interrupt-route
    presence alone.
14. If the same `i2c` driver instance claims capability tag
    `embedded-hal-async-i2c-master`, it must also carry the explicit
    event/error interrupt routes plus any required controller-local
    interrupt-enable handles named by `i2cMasterBindings`. That async claim
    does not authorize slave mode, SMBus behavior, or 10-bit addressing.
15. If an `i2c` driver instance names `loweringPattern = "legacy-event-i2c-slave"`,
    the same driver instance must also carry explicit `i2cSlaveBindings`
    naming the direct own-address programming, direction, packet-boundary, data,
    and clear roles used by the HAL-specific 7-bit slave packet path. Families
    that split the programmed own address across multiple register bits must use
    the explicit programming contract named by `i2cSlaveBindings`; the selected
    family may not be reconstructed from vendor register names, from pin routes
    alone, or from interrupt-route presence alone.
16. If the same `i2c` driver instance claims capability tag
    `embassy-async-i2c-slave`, it must also carry the explicit event/error
    interrupt routes plus any required controller-local interrupt-enable
    handles named by `i2cSlaveBindings`. That async claim remains HAL-specific
    and does not authorize general-call, dual-address, SMBus, or 10-bit slave
    behavior.
17. If the same `i2c` driver instance claims capability tag
    `embassy-i2c-slave-isr-dispatch`, it must also claim
    `embassy-async-i2c-slave`, and the approved packet-boundary closure
    must justify an ISR-level completed RX-packet callback path with a
    user-supplied static buffer on top of the same async slave contract rather
    than a separate byte-stream callback API.
18. A `usb-device` driver instance may claim only the controller-level helper
...
19. If a `usb-device` driver instance names a `loweringPattern`, that selector
...
20. If an `adc` driver instance names `loweringPattern = "regular-sequence-adc-dma"`,
    ...
    DMA helpers.
18. If a `dma` driver instance carries `dmaAsyncBindings`, the same driver
...
19. If a `flash` driver instance names `loweringPattern = "stm32f1-page-flash"`,
    ...
    explicitly.
20. If a driver instance claims `embassy-time-driver`, `timeDriverSource`
...
21. A `hardware-timer` time-driver claim must remain rooted in a `timer`
...
22. If a hardware-timer time-driver claim uses a timer family whose supported
...
23. A non-SysTick time-driver claim must narrow the generated time base to one
...
24. A `systick` time-driver claim must remain rooted in an `interrupt` driver
    instance whose approved route inventory explicitly targets SysTick.
25. A `systick` time-driver claim must not declare `timeDriverTickHz`; that
    rate is defined by the SysTick lowering path itself.
26. A generated non-SysTick time-driver path must preserve the approved
    ...
    `timeDriverTickHz`.
27. `systemResetOperationRef` is valid only on an `interrupt` driver instance.
    Its referenced operation must target the same interrupt-controller lowering
    path and have one unambiguous final `write` step with both `targetRef` and
    integer `value`. Earlier explicit steps are allowed. The generator must
    reject missing, ambiguous, or non-terminal reset sequences rather than
    infer the reset register, key, bit, or value.
28. Host-emulated lowering must expose the same reset request through the
    interrupt driver's paired emulator/state handle as an observable,
    deterministic reset event. The generated `HostEmulator` exposes
    `system_reset_count(driver_id)` for this observation. It must not terminate
    the host process or claim to reset external board components.
