...
| `can` | supported | First-cut CAN lowering targets classic CAN 2.0 controller behavior only. The generated surface may expose `embedded-can`-aligned blocking TX/RX plus controller, mailbox/FIFO, interrupt, and filter-bank helpers only when those behaviors are backed by explicit topology, semantic, and structural lowering inputs. CAN FD, time-triggered CAN, and advanced low-power/wakeup behavior remain out of scope unless explicitly added in a later contract revision. |
...
| `can` | clock/reset support for emitted bring-up helpers; explicit semantic operations and/or control refs for controller initialization, mode transitions, transmit request, receive release/acknowledge, and any claimed interrupt enable/acknowledge path; interrupt topology routes for any emitted interrupt-facing helpers; structural register/field data for controller state, bit timing, mailbox/FIFO data paths, identifier/DLC fields, status/error bits, and any emitted filter-bank configuration helpers. First-cut filter support must remain traceable to explicitly modeled filter-bank extent and configuration registers rather than inferred family defaults. |
...
| `can` | Bring-up and mode/configuration helpers plus only those classic-CAN controller, mailbox/FIFO, filter-bank, and interrupt-facing methods whose control/data paths are explicitly modeled. A first-cut generated CAN driver may expose an `embedded-can`-aligned blocking surface together with HAL-specific controller/filter helpers, but it must not claim CAN FD, time-triggered CAN, or inferred low-power/wakeup semantics without an explicit future contract extension. |
